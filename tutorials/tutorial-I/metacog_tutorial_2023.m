%% UCL-Paris Summer School Consciousness
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% 
% _FUNCTION:_ ss_type2_SDT_sim(d, noise, c, Nratings, Ntrials);
...
% 
% _FUNCTION:_ ss_plot_confidence(data, title, type)
% 
...
sim2.dHigh.responses = ss_type2_SDT_sim(sim2.dValueHigh, sim2.noise.value, sim2.c, sim2.Nratings, sim2.Ntrials);
sim2.dLow.responses = ss_type2_SDT_sim(sim2.dValueLow, sim2.noise.value, sim2.c, sim2.Nratings, sim2.Ntrials);
%% 
% 
% 
% *Plot the confidence results for the two simulations:*
...
% 
% _FUNCTION:_ ss_plot_type2roc(data, title, type)
% 
% _NOTE:_ type = 'obs' or 'est' (observed or estimated data from model)
...
% 
% _FUNCTION:_ ss_calcAU_type2roc(data)
...
% 
% _FUNCTION:_ ss_metad_sim(d, meta-d', c, Nratings, Ntrials);
...
% and stimulus 2 (nR_S2) for each confidence level, and are generated by ss_metad_sim
...
for a = 1:length(sim5.params.meta_d)
    for b = 1:sim5.params.Nsims
        ...
        % Simulate data
        sim5.data{b,a} = ss_metad_sim(sim5.values{a}.d(b), sim5.values{a}.meta_d(b), sim5.values{a}.c(b), sim5.params.Nratings, sim5.params.Ntrials);
...
% 
% _FUNCTION:_ ss_plot_simVfit(data, data.fit, parameter, model)
% 
...
for a = 1:length(sim6.params.meta_d)
    for b = 1:sim6.params.Nsims
        ...
        sim6.data{b,a} = ss_metad_sim(sim6.params.d, sim6.params.meta_d(a), sim6.params.c, sim6.params.Nratings, sim6.params.Ntrials);
...
for a = 1:2
    for b = 1:simTwoGroupsI.params.Nsims(a)
        ...
        % Simulate data
        simTwoGroupsI.data{a}.sims{b} = ss_metad_sim(simTwoGroupsI.values{a}.d(b), simTwoGroupsI.values{a}.meta_d(b), simTwoGroupsI.values{a}.c(b), simTwoGroupsI.params.Nratings, simTwoGroupsI.params.Ntrials);
...
for b = 1:simTwoGroupsD.params.Nsims
    ...
    for a = 1:2
        ...
        % Simulate data
        simTwoGroupsD.data{a}.sims{b} = ss_metad_sim(simTwoGroupsD.values{a}.d(b), simTwoGroupsD.values{a}.metad(b), simTwoGroupsD.values{a}.c(b), simTwoGroupsD.params.Nratings, simTwoGroupsD.params.Ntrials);
...
for a = 1:simRegression.params.Nsims
    simRegression.data{a} = ss_metad_sim(simRegression.values.d(a), simRegression.values.meta_d_adjusted(a), simRegression.values.c(a), simRegression.params.Nratings, simRegression.params.Ntrials);
...
for a = 1:simRC.params.Nsims
    simRC.data{a} = ss_type2_SDT_sim(simRC.values.d(a), simRC.params.noise, simRC.values.c(a), simRC.params.Nratings, simRC.params.Ntrials);
